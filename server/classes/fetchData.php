...
class fetchData extends DbConnection
{
    ...
    {
        $sql = "SELECT * FROM course AS a INNER JOIN courseallocation AS c ON a.id = c.courseid INNER JOIN lecturer AS l ON c.lecturerid = l.id WHERE l.id = '{$_SESSION["lecturerid"]}'";
        $qry = $this->connection->prepare($sql);
        $exec = $qry->execute(array());
    ...
    {
        $sql = "SELECT * FROM quiz WHERE lecturerid = '{$_SESSION['lecturerid']}'";
        $qry = $this->connection->prepare($sql);
        $exec = $qry->execute(array());
