...
class insertData extends DbConnection {
    ...
    public function insertQuestion($examid,$questiontitle,$option1,$option2,$option3,$option4,$answer,$questionno){
        $sql = "INSERT INTO question (examid,option1, questiontitle,option2,option3,option4,correct,questionno) VALUES (:examid,:option1,:questiontitle,:option2,:option3,:option4,:correct,:questionno)";
        $query = $this->connection->prepare($sql);
        $exec = $query->execute(array(':questiontitle'=>$questiontitle,':examid'=>$examid,':option1'=>$option1,':option2'=>$option2,':option3'=>$option3,':option4'=>$option4,':correct'=>$answer,':questionno' => $questionno));
        if ($query->errorCode()==0) {
            return array('status'=>1);
        }else{
            return array('status'=>0, 'message'=>$query->errorInfo());
